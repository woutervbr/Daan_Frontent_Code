...
import { getNextPaginaNumber } from "../context/PaginaContext";
...
const calculateAllPages = (groupedData) => {
  let currentPage = 3; // Page 1: Cover, Page 2-3: TOC
  const chapterPageMap = {};

  Object.keys(groupedData).forEach((chapterId) => {
    currentPage += 1; // For chapter title + image
    ...
    Object.keys(chapter.questions).forEach((questionId) => {
      const messageCount = chapter.questions[questionId].messages.length;
      pages.push({
        type: "content",
        count: messageCount,
      });
      currentPage += messageCount;
    });
...
const SimpleFinalizebook = () => {
  ...
  const [paginaNumber, setPaginaNumber] = useState(null);
...
const [isMobile, setIsMobile] = useState(false);
  ...
  // Calculate when groupedData changes
  ...
  //       console.log(
  //         height,
  //         "heightheightheightheightheightheightheightheightheightheight"
  //       );
  ...
  async function postData() {
    try {
      ...
      const filterData = data.data?.filter((doc) => {
        const hasChapter = !!doc?.chapterId; // chapterId exists and is not null
        const isStamboom = doc?.chapterId?.chapterTitle === "Stamboom";
        const hasMessages = doc?.messages?.length > 0;

        // remove entries without a valid chapterId object
        return hasChapter && (isStamboom || hasMessages);
      });
